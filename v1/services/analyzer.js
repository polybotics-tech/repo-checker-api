...
import {
  ...
  readGemfile,
} from "../../ruby-config/helper.js";

export async function githubRepoAnalyzer(url) {
  ...
  console.log("[action] extracting package paths (npm & ruby)...");
  const packageJsonPaths = extractPackageJsonPathsFromTree(tree);
  const gemfilePaths = extractGemfilePathsFromTree(tree);

  console.log("[log] package.json paths: ", packageJsonPaths);
  console.log("[log] Gemfile paths: ", gemfilePaths);

  if (!packageJsonPaths.length && !gemfilePaths.length) {
    throw new NotFoundError("No package.json or Gemfile found in this repository");
  }

  // Trimming if too many
  const totalFiles = packageJsonPaths.length + gemfilePaths.length;
  if (totalFiles > 10) {
    console.warn("[warning] many manifest files detected. trimming...");
    // maybe trim each to 5 max
    if (packageJsonPaths.length > 5) packageJsonPaths.splice(5);
    if (gemfilePaths.length > 5) gemfilePaths.splice(5);
  }

  console.log("[action] reading all package files...");

  const npmPackageList = await Promise.all(
    packageJsonPaths.map(async (path) => {
      let packageJson = await readPackageJson({ owner, repo, path });
      if (!packageJson) return { path, dependencies: [], devDependencies: [] };
      let { dependencies, devDependencies } = extractDependencies(packageJson);
      return { path, dependencies, devDependencies };
    }),
  );

  const rubyPackageList = await Promise.all(
    gemfilePaths.map(async (path) => {
      const gemfileContent = await readGemfile({ owner, repo, path });
      if (!gemfileContent) return { path, dependencies: [], devDependencies: [] };
      const { dependencies, devDependencies } = extractGemDependencies(gemfileContent);
      return { path, dependencies, devDependencies };
    }),
  );
